...
function App() {
  ...
  const findOutfit = (dressCode) => {
    console.log(dressCode)
    let filteredItems = clothing.filter((e) => {
      return e.dressCode === dressCode
    })
    
    let topChoices = filteredItems.filter((e) => {
      return e.type === 'top'
    })
    let shoesChoices = filteredItems.filter((e) => {
      return e.type === 'shoes'
    })
    let bottomsChoices = filteredItems.filter((e) => {
      return e.type === 'bottom'
    })
    setTop(topChoices[Math.floor(topChoices.length * Math.random())])
    setBottoms(bottomsChoices[Math.floor(bottomsChoices.length * Math.random())])
    setShoes(shoesChoices[Math.floor(shoesChoices.length * Math.random())])
  }
  ...
  return (
    <div className="App App-header">
      <div id='header'>
        <h1>Welcome to your outfit planner!</h1>
        <h3>Please choose an outfit style</h3>
      </div>
      ...
      <div id='results'>
        ...
        <img src={bottoms.imageUrl}></img>
        <h3>{bottoms.description}</h3>
